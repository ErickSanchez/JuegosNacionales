<div id="left-column">
   ...
   <?php   }?>
	<div class="menu round">
		<h3 class="section-title round">DEPORTES</h3>
		<ul>
			<?php 
				foreach($sports as $row)
					echo '<li>'.anchor('estadisticas/tabla_general/'.ucfirst(mb_strtolower($row->sportName)).'/',ucfirst(mb_strtolower($row->sportName, 'UTF-8')), 'title="'.ucfirst(mb_strtolower($row->sportName)).'"').'</li>';
			?>
		</ul>
	</div>
	<div class="menu round">
